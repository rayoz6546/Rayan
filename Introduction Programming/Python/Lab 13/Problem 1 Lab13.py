# -*- coding: utf-8 -*-
"""
Created on Wed Apr 29 00:32:15 2020

@author: RAYAN
"""
# i copied the first code as it is from the moodle page
class Queue(object):
...
class UnboundedQueue(Queue):
    def __doubleList(self): 
        new_list=[None]*(2*self.maxSize)
        counter=0   
        _index_=self.head
                     
        while counter!=self.size: 
            new_list[counter]=self.L[_index_]
            if _index_<self.maxSize-1:
                _index_+=1
            else: 
                _index_=0
            counter=counter+1
