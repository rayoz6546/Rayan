...
int main() {
	...
	cpuA = (float)execTimeA/periodA;
	cpuB = (float)execTimeB / periodB;
	cpuUtil = cpuA + cpuB;
    ...
    for (T = 0; T <= 200; T++) {
        ...
        if (cpuUtil > 1) {
			printf("CPU cannot scedule task set!\n");
			return EXIT_FAILURE;
		}
        ...
        if (T % periodA == 0 && T % periodB != 0) {
            ...
            if (tB < execTimeB) { // process B is unfinished yet
                ...
                if (periodA <= periodB){
	                	if (T+execTimeA+execTimeB-tB > absDeadlineB){			
			...
			if (T>absDeadlineB) {printf("process B missed deadline! Not schedulable\n"); return EXIT_FAILURE; } 
        ...
        if (T % periodA != 0 && T % periodB == 0) {
            ...
            if (tA < execTimeA) { // process A is unfinished yet
				if (periodB < periodA){
	                // Your code here to handle preemption
						if (T+execTimeB+execTimeA-tA > absDeadlineA){			
			...
			if (T>absDeadlineA) {printf("process A missed deadline! Not schedulable\n"); return EXIT_FAILURE; } 
